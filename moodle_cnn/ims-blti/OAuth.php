...
class OAuthToken {
   ...
   * csc - genera la serialización de cadena básica de una señal que un servidor respondería a la llamada request_token y señal_acceso 
   */
  function to_string() {
    return "oauth_token=" .
...
class OAuthSignatureMethod_RSA_SHA1 extends OAuthSignatureMethod {
  ...
  public function build_signature(&$request, $consumer, $token) {
    ...
    // Fetch the private key cert based on the request / csc-Obtiene el certificado de clave privada basada en la solicitud
    $cert = $this->fetch_private_cert($request);

    // Pull the private key ID from the certificate / csc-Lanza la llave ID privada del certificado
    $privatekeyid = openssl_get_privatekey($cert);

    // Sign using the key
    $ok = openssl_sign($base_string, $signature, $privatekeyid);

    // Release the key resource / csc-Libera la clave del recurso
  ...
  public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) {
    ...
    if (!$parameters) {
     ...
     // Add POST Parameters if they exist
      ...
      // csc-Tenemos una Autorización-cabecera con los datos de OAuth. Analizamos la cabecera y añadimos o anulamos cualquier duplicado de GET o POST
